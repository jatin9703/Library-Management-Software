...
const{staff_info,student_info,recent,material_info,history} =require("./database.js")
...
//functions to give sms to late returns


async function givealerts(){
  try {
    // For Books
    ...
    try {
      ...
      for (let i = 0; i < allBooks.length;i++) {
        const updatedstudent=await student_info.find({crn:allBooks[i].student.crn})
        const stud = await student_info.updateOne({crn:allBooks[i].student.crn}, {$inc:{fine:50}})
        twilio.sendsms("K.K. WAGH LIBRARY , Kindly Submit Your Issued Book !! Book Name "+allBooks[i].material.material_name+" Issued On "+allBooks[i].issue_date+" Return Date "+allBooks[i].return_date+" Rs 50 fine has been imposed!!! ",allBooks[i].student.mob);
    }
   ...
   try {
    ...
    for (let i = 0; i < allMagazines.length;i++) {
    
      twilio.sendsms("K.K. WAGH LIBRARY , Kindly Submit Your Issued Magazine !! Magazine Name "+allMagazines[i].material.material_name+" Issued On "+allMagazines[i].issue_date+" Return Date "+allMagazines[i].return_date+" Rs 50 fine has been imposed!!! ",allMagazines[i].student.mob);
  }
   } catch (error) {
    ...
    try {
      ...
      for (let i = 0; i < allResearchPapers.length;i++) {
        
        twilio.sendsms("K.K. WAGH LIBRARY , Kindly Submit Your Issued Research Paper !! Research Paper Name "+allResearchPapers[i].material.material_name+" Issued On "+allResearchPapers[i].issue_date+" Return Date "+allResearchPapers[i].return_date+" Rs 50 fine has been imposed!!! ",allResearchPapers[i].student.mob);
    }
    } catch (error) {
    ...
    const found_staff= staff_info.findOne({staffcrn:username}).then((user)=>{
      ...
      if(!user){
        ...
        if(password==user.password){
          ...
          if(user.role==="administrator"){res.send(`<script>alert("Welcome Admin , Directing To Administartor Page!"); window.location="/staff/?username=${user.staffname}&role=${user.role}";</script>`)
        }
        else{
...
app.get("/remain",async(req,res)=>{
  ...
  try {
    ...
    try {
      ...
      console.log("no book on return")
    }
   try {
    console.log('All Magazines:', allMagazines);
    const r=allMagazines[0].return_date
   } catch (error) {
    console.log("no magzine to return")
   }
    try {
      console.log('All Research Papers:', allResearchPapers);
      const r=allResearchPapers[0].return_date
